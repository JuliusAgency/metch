    ...
    <div className="max-w-4xl mx-auto" dir="rtl" onDragEnter={handleDrag}>
      ...
      <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
        ...
        <div className="relative h-32">
          ...
          <button className="absolute top-4 left-6 w-10 h-10 bg-white/30 rounded-full flex items-center justify-center backdrop-blur-sm hover:bg-white/50 transition-colors z-10">
        ...
        <div className="px-8 py-6 mt-4">
          ...
          <div className="text-center mb-6">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">{jobData.title || 'מנהלת קישורי לקוחות'}</h2>
            
            {/* Main Job Info Badges */}
            <div className="flex justify-center items-center gap-4 mb-4">
              <Badge variant="outline" className="flex items-center gap-2 text-blue-600 border-blue-300 px-4 py-2">
                <span>מיידי</span>
                <Clock className="w-4 h-4" />
              </Badge>
              <Badge variant="outline" className="flex items-center gap-2 text-blue-600 border-blue-300 px-4 py-2">
                <span>{employmentTypeText[jobData.employment_type] || 'משרה מלאה'}</span>
            ...
            <div className="flex justify-center gap-2 mb-8 flex-wrap">
              ...
              jobData.company_perks.slice(0, 4).map((perk, index) => {
                ...
                return (
                  <Badge key={index} className={`${colors[index % colors.length]} border-0 px-4 py-2`}>
                      {perk}
                    </Badge>);
              ...
              <>
                  <Badge className="bg-green-100 text-green-800 border-0 px-4 py-2">משרה מעניין</Badge>
                  <Badge className="bg-blue-100 text-blue-800 border-0 px-4 py-2">בנק חבילת</Badge>
                  <Badge className="bg-purple-100 text-purple-800 border-0 px-4 py-2">עבודה במשמרות</Badge>
                  <Badge className="bg-orange-100 text-orange-800 border-0 px-4 py-2">סביבה</Badge>
                </>
              }
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
            ...
            <div className="text-right">
              <h3 className="font-bold text-lg mb-4 text-gray-900">חומרי אימון</h3>
              <div className="space-y-2 text-gray-700">
                {jobData.structured_education && jobData.structured_education.length > 0 ?
                jobData.structured_education.map((edu, index) =>
                <p key={index} className="text-sm">• {edu.value}</p>
                ) :

                <div className="space-y-2">
                    <p className="text-sm">• ניסיון של שנתיים לפחות בחברת הפקות מומלץ -יתרון</p>
                    <p className="text-sm">• ניסיון קודם בתחום הרפואה או הביטוח או בתחום סמוך -יתרון</p>
                    <p className="text-sm">• יכולת עבודה במסגרת הקשרת מורכבת (אופציה באטלבוקס)</p>
                    <p className="text-sm">• עבודה בצוותי בעלות (אופציה באטלבוקס)</p>
            ...
            <div className="text-right">
              ...
              <div className="space-y-2 text-gray-700">
                {jobData.structured_requirements && jobData.structured_requirements.length > 0 ?
                jobData.structured_requirements.map((req, index) =>
                <p key={index} className="text-sm">• {req.value}</p>
                ) :
            ...
            <div className="text-right">
              ...
              <div className="space-y-2 text-gray-700">
                ...
                <div className="space-y-2">
                    <p className="text-sm">ובאברחי ה Techwise-מאיתנו ה מתפטמויות רב/חלק נקיות מכלבובלוצאות ביחידו/הו לשמתר לצברת מטוני</p>
                    <p className="text-sm">התפקיד כולל ניהולת האמצאי של המידתי ברחמרקני במקומם, התמחות בטכנולוגיות וההוצאות במטלגיטכיולג יחרבמן ראלויות מחירות פיקיותולחי. המטמלות לחנותיות ו חתמות המניוחדות עווין ווית פרויקטים</p>
                  </div>
                }
              </div>
            </div>
          ...
          <div className="flex justify-center gap-4 hidden">
            ...
            <Button variant="outline" className="border-gray-300 text-gray-700 hover:bg-gray-50 px-8 py-3 rounded-full font-bold">
              חזרה
            </Button>
          </div>
        </div>

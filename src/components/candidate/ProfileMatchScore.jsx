const ProfileMatchScore = ({ matchScore }) => (
    <div className="w-full max-w-3xl mx-auto">
        <div className="relative w-full h-5 bg-gray-200 rounded-full overflow-hidden">
            <div
                className={`absolute top-0 right-0 h-full flex items-center justify-center transition-all duration-500 ${matchScore >= 70 ? 'bg-green-400' : matchScore >= 40 ? 'bg-orange-400' : 'bg-red-500'}`}
            ...
            <div className="absolute inset-0 flex items-center justify-center text-[10px] font-bold text-[#003566]">
                {matchScore}% התאמה
            </div>
        </div>
    </div>
